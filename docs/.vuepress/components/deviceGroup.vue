            ...
            <div class="devHead">
                <router-link :to="dev.hasJailbreaks ? url : ''" :style="{
                    ...
                    <div :class="['flexWrapper','flexImg',dev.expanded ? 'expanded' : 'small']" style="user-select: none;">
                        <div>
                            <template v-if="dev.img.count > 0"><img v-for="i in Math.min(dev.img.count,3)" :key="i" :class="['devImage', `devImage${i}`, dev.expanded ? 'expanded' : 'small']" :src="`https://img.appledb.dev/device@preview/${dev.img.key}/${dev.img.names[i-1]}${isDarkMode && dev.img.dark ? '_dark' : ''}.webp`"></template>
