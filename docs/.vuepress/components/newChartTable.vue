<template>
  <div v-html="getDeviceInfo()"/>
  <div v-html="getDeviceTable()"/>
</template>
...
function getDeviceTable() {
  const d = props.device;
  if ((!d || !deviceList.hasOwnProperty(d)) && !props.showAll) return;
  ...
  for (var i in iosList)
    if (iosList[i].hasOwnProperty('devices'))
      if (iosList[i].devices.includes(d) || props.showAll)
  ...
  for (var b in buildArr) {
    ...
    html += '<td>'
    const jbArr = getJailbreaks(buildArr[b], d)
    for (var jb in jbArr) {
  ...
  for (var jb in jbList)
    for (var compatList in jbList[jb])
      if (jbList[jb][compatList].hasOwnProperty('firmwares') && jbList[jb][compatList].hasOwnProperty('devices'))
        if (jbList[jb][compatList].firmwares.includes(os.build) && (jbList[jb][compatList].devices.includes(d) || props.showAll))
