      ...
      <tr>
        ...
        <template v-for="majFw in iosVer"><template v-for="midFw in majFw"><template v-for="minFw in midFw"><template v-for="v in minFw">
          <tr v-if="v.devices.includes(d)">
            ...
            <td>
              <template v-for="tool in jbList"><template v-for="compat in tool"><template v-if="compat.hasOwnProperty('firmwares') && compat.hasOwnProperty('devices')"><template v-if="compat.firmwares.includes(v.build) && compat.devices.includes(device || d)">
                <a :target="(tool.guideLink) ? '_self': '_blank'" :href="tool.guideLink || tool.toolLink" v-text="tool.guideName || tool.toolName"/>
